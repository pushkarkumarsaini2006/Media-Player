...
class LuminexPlayer {
        ...
        this.videos = [
            {
                ...
                title: 'Flower Motion',
                description: 'A calm macro nature clip with smooth camera motion and vivid color.',
            ...
            },
            {
                ...
                title: 'Big Buck Clip',
                description: 'A short animated clip used widely for browser media demos.',
                poster: 'https://images.unsplash.com/photo-1522869635100-9f4c5e86aa37?w=800&h=450&fit=crop',
                duration: '0:10',
                views: '1.1M',
                likes: '24K',
                year: '2018'
            },
            {
                ...
                title: 'Sample Reel',
                description: 'A short sample reel for testing quick playback and controls.',
    ...
    setupVideoPlayer() {
        ...
        this.mainVideo?.addEventListener('timeupdate', () => {
            ...
            const progress = duration && Number.isFinite(duration)
                ? (this.mainVideo.currentTime / duration) * 100
                : 0;
        ...
        progressBar?.addEventListener('click', (e) => {
            ...
            if (this.mainVideo.duration && Number.isFinite(this.mainVideo.duration)) {
                this.mainVideo.currentTime = pos * this.mainVideo.duration;
            }
        });

        this.mainVideo?.addEventListener('error', () => {
            this.showToast('Video could not be loaded. Trying the next one...');
            this.changeVideo(1);
        });
    ...
    setupAudioPlayer() {
        ...
        this.audioPlayer?.addEventListener('timeupdate', () => {
            ...
            const progress = duration && Number.isFinite(duration)
                ? (this.audioPlayer.currentTime / duration) * 100
                : 0;
        ...
        this.audioPlayer?.addEventListener('error', () => {
            this.showToast('Track failed to load. Trying next track...');
            this.changeTrack(1);
        });
    }

    safePlayVideo() {
        if (this.videoPlayPromise !== null) {
            this.videoPlayPromise.catch(() => {});
        }

        this.videoPlayPromise = this.mainVideo.play();
        return this.videoPlayPromise.then(() => {
            this.videoPlayPromise = null;
            return true;
        }).catch(error => {
            this.videoPlayPromise = null;
    ...
    startVisualizer() {
        if (!this.audioContext) {
            try {
                const currentSrc = this.audioPlayer.currentSrc || this.audioPlayer.src;
                if (currentSrc) {
                    const sourceUrl = new URL(currentSrc, window.location.href);
                ...
                case 'share':
                    {
                        ...
                        const shareUrl = isWebContext ? window.location.href : 'https://media-player-ten-self.vercel.app/';
                        ...
                        if (navigator.share) {
                            ...
                            this.showToast('Sharing is limited in local file mode. Open via a local server for full sharing.');
                        }
                    }
                    break;
            }
            ...
            switch (e.key.toLowerCase()) {
                ...
                case 'arrowleft':
                    if (document.querySelector('#music-section.active')) {
                        if (Number.isFinite(this.audioPlayer.duration) && this.audioPlayer.duration > 0) {
                            this.audioPlayer.currentTime = Math.max(0, this.audioPlayer.currentTime - 10);
                        }
                    } else {
                        if (Number.isFinite(this.mainVideo.duration) && this.mainVideo.duration > 0) {
